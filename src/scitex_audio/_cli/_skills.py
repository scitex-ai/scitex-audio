"""`scitex-audio skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-audio/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-audio/` directory."""
    import scitex_audio

    pkg_dir = Path(scitex_audio.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-audio/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    """Agent-facing skills bundled with scitex-audio.

    \b
    Examples:
      $ scitex-audio skills list
      $ scitex-audio skills get 01_installation
      $ scitex-audio skills install                  # → ~/.scitex/dev/skills/scitex-audio/
      $ scitex-audio skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-audio skills get 01_installation
      $ scitex-audio skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-audio/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Default: symlink the entire `_skills/scitex-audio/` dir to
    ~/.scitex/dev/skills/scitex-audio/ so add/rename/delete in
    source propagates immediately.

    \b
    Example:
      $ scitex-audio skills install
      $ scitex-audio skills install --claude-symlink
      $ scitex-audio skills install --no-link --dest /tmp/scitex-audio-skills
    """
